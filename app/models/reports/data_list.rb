...
module Report
  class DataList < Base
    ...
    def most_used_answers
      ...
      answers = Answer.joins(:answer_stores).group('qwester_answers.id').order('COUNT(*) DESC').limit(ITEMS_TO_RETURN).count
    ...
    def most_used_questionnaires
      ...
      questionnaires = Questionnaire.joins(:answer_stores).group('qwester_questionnaires.id').order('COUNT(*) DESC').limit(ITEMS_TO_RETURN).count
